...
class Model(torch.nn.Module):
    def __init__(self, wavelet_affine):
        ...
        self.wavelte_trans = wavelet_trans.Wavelet
        ...
        self.coding_LL_Y = PixelCNN.PixelCNN()
        self.coding_HL_list_Y = torch.nn.ModuleList([PixelCNN.PixelCNN_Context(1) for _i in range(self.trans_steps)])
        self.coding_LH_list_Y = torch.nn.ModuleList([PixelCNN.PixelCNN_Context(2) for _i in range(self.trans_steps)])
        self.coding_HH_list_Y = torch.nn.ModuleList([PixelCNN.PixelCNN_Context(3) for _i in range(self.trans_steps)])

        self.coding_LL_U = PixelCNN.PixelCNN_Context(1)
        self.coding_HL_list_U = torch.nn.ModuleList(
            [PixelCNN.PixelCNN_Context(1 + 1) for _i in range(self.trans_steps)])
        self.coding_LH_list_U = torch.nn.ModuleList(
            [PixelCNN.PixelCNN_Context(2 + 1) for _i in range(self.trans_steps)])
        self.coding_HH_list_U = torch.nn.ModuleList(
            [PixelCNN.PixelCNN_Context(3 + 1) for _i in range(self.trans_steps)])

        self.coding_LL_V = PixelCNN.PixelCNN_Context(2)
        self.coding_HL_list_V = torch.nn.ModuleList(
            [PixelCNN.PixelCNN_Context(1 + 2) for _i in range(self.trans_steps)])
        self.coding_LH_list_V = torch.nn.ModuleList(
            [PixelCNN.PixelCNN_Context(2 + 2) for _i in range(self.trans_steps)])
        self.coding_HH_list_V = torch.nn.ModuleList(
            [PixelCNN.PixelCNN_Context(3 + 2) for _i in range(self.trans_steps)])
    ...
    def forward(self, x, alpha, train, scale_init, wavelet_trainable, coding=1):
        ...
        if not coding:
            ...
            bits = self.coding_LL_Y(LL[:, 0].unsqueeze(1))
            bits += self.coding_LL_U(LL[:, 1].unsqueeze(1), LL[:, 0].unsqueeze(1))
            bits += self.coding_LL_V(LL[:, 2].unsqueeze(1),
                                     torch.cat((LL[:, 0].unsqueeze(1), LL[:, 1].unsqueeze(1)), 1))
        LL = LL * self.scale
        ...
        for i in range(self.trans_steps):
            ...
            if coding:
                bits = bits + self.coding_HL_list_Y[j](HL_list[j][:, 0].unsqueeze(1), LL[:, 0].unsqueeze(1))
                bits = bits + self.coding_HL_list_U[j](HL_list[j][:, 1].unsqueeze(1),
                                                       torch.cat((LL[:, 1].unsqueeze(1), HL_list[j][:, 0].unsqueeze(1)),
                                                                 1))
                bits = bits + self.coding_HL_list_V[j](HL_list[j][:, 2].unsqueeze(1), torch.cat(
                    (LL[:, 2].unsqueeze(1), HL_list[j][:, 0].unsqueeze(1), HL_list[j][:, 1].unsqueeze(1)), 1))
            HL_list[j] = HL_list[j] * self.scale

            if coding:
                bits = bits + self.coding_LH_list_Y[j](LH_list[j][:, 0].unsqueeze(1),
                                                       torch.cat((LL[:, 0].unsqueeze(1), HL_list[j][:, 0].unsqueeze(1)),
                                                                 1))
                bits = bits + self.coding_LH_list_U[j](LH_list[j][:, 1].unsqueeze(1), torch.cat(
                    (LL[:, 1].unsqueeze(1), HL_list[j][:, 1].unsqueeze(1), LH_list[j][:, 0].unsqueeze(1)), 1))
                bits = bits + self.coding_LH_list_V[j](LH_list[j][:, 2].unsqueeze(1), torch.cat((LL[:, 2].unsqueeze(1),
                                                                                                 HL_list[j][:,
                                                                                                 2].unsqueeze(1),
                                                                                                 LH_list[j][:,
                                                                                                 0].unsqueeze(1),
            ...
            if coding:
                bits = bits + self.coding_HH_list_Y[j](HH_list[j][:, 0].unsqueeze(1), torch.cat(
                    (LL[:, 0].unsqueeze(1), HL_list[j][:, 0].unsqueeze(1), LH_list[j][:, 0].unsqueeze(1)), 1))
                bits = bits + self.coding_HH_list_U[j](HH_list[j][:, 1].unsqueeze(1), torch.cat((LL[:, 1].unsqueeze(1),
                                                                                                 HL_list[j][:,
                                                                                                 1].unsqueeze(1),
                                                                                                 LH_list[j][:,
                                                                                                 1].unsqueeze(1),
                                                                                                 HH_list[j][:,
                                                                                                 0].unsqueeze(1)), 1))
                bits = bits + self.coding_HH_list_V[j](HH_list[j][:, 2].unsqueeze(1), torch.cat((LL[:, 2].unsqueeze(1),
                                                                                                 HL_list[j][:,
                                                                                                 2].unsqueeze(1),
                                                                                                 LH_list[j][:,
                                                                                                 2].unsqueeze(1),
