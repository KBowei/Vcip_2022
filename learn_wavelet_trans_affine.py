...
class P_block(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_logscale_for_affine = torch.nn.Conv2d(16, 3, 3, 1, 0)
    ...
    def forward(self, x):
        ...
        x4 = x1 + x3
        shift = self.conv4(self.padding_reflect(x4))
        log_scale = self.conv_logscale_for_affine(self.padding_reflect(x4))
...
class learn_lifting97(torch.nn.Module):
    ...
    def forward_trans(self, L, H):
        ...
        H = H * (lifting_coeff[4] + 0 * self.leran_wavelet_rate)
        L = L * (lifting_coeff[5] + 0 * self.leran_wavelet_rate)
