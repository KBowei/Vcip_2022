...
class PixelCNN(torch.nn.Module):
    ...
    def forward(self, x, range_low_bound, range_high_bound):
        range = torch.arange(start=range_low_bound, end=range_high_bound+1, dtype=torch.float, device = torch.device('cuda'))
        range = range.unsqueeze(0).unsqueeze(0).unsqueeze(0)
        x = self.conv_pre(x)
        conv1 = x
        x = self.res1(x)
        x = self.res2(x)
        ...
        params = self.infer(x)
        params = params.repeat(1, 1, 1, range_high_bound-range_low_bound+1)
        
        prob = self.gaussin_entropy_func(range, params)
...
class PixelCNN_Context(torch.nn.Module):
    ...
    def forward(self, x, context, range_low_bound, range_high_bound):
